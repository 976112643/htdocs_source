...
<aside class="main-sidebar">
 ...
 foreach($page_child as $v){ if($v['status']==1){ switch($v['title']){ case '删除': ?>
																...
																<?php
 break; case '禁用'; ?>
																<a href="<?=U('disable',array('status'=>0))?>" class="btn btn-danger btn-sm ajax-post" target-form="ids">禁用</a>
																<?php
 break; case '启用'; ?>
										<a href="<?=U('enable',array('status'=>1))?>" class="btn btn-info btn-sm ajax-post" target-form="ids">启用</a>
										<?php
 break; case '添加'; ?>
										...
										<table id="example2" class="table table-bordered table-hover dataTable text-center" role="grid" aria-describedby="example2_info">
											<thead>
												<tr role="row">
													...
													<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending">标题</th>
													...
													<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">IP</th>
 ...
 $menu_all = get_result('menu',array(),'title,url,id,pid'); $menu_all = list_to_tree($menu_all); $menu_temp=array(); $i=0; foreach($menu_all as $val){ $menu_temp[$i]['url'] = strtolower($val['url']); $menu_temp[$i]['title'] = $val['title']; $i++; foreach($val['_child'] as $v){ $menu_temp[$i]['url'] = strtolower($v['url']); $menu_temp[$i]['title'] = $val['title'].'->'.$v['title']; $i++; foreach($v['_child'] as $_v){ $menu_temp[$i]['url'] = strtolower($_v['url']); $menu_temp[$i]['title'] = $val['title'].'->'.$v['title'].'->'.$_v['title']; $i++; } } } foreach($result as $row){ if($row['action_url']=='backend/base/public/login'){ $row['log_title']='登录系统'; }else{ foreach($menu_temp as $val){ if($row['action_url']==$val['url']){ $row['log_title']=$val['title']; } } } ?>
												<tr>
													...
													<td><?=$row['log_title']?></td>
													<td>
														<?php  $json=$row['description']; $json=htmlspecialchars_decode($json); $description=json_decode($json); foreach($description as $_row){ echo '[ '.$_row->id.' ] '.$_row->title.'<br/>'; } ?>
													</td>

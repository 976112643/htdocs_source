...
<aside class="main-sidebar">
 ...
 foreach($page_child as $v){ if($v['status']==1){ switch($v['title']){ case '删除': ?>
                        	<a href="<?=U('del',array('status'=>-1,'ids'=>$row['id']))?>" class="btn btn-danger btn-sm ajax-post confirm " target-form="ids">删除</a>
                        	<?php
 break; case '禁用'; ?>
						...
						<table id="example2" class="table table-bordered table-hover dataTable text-center" role="grid" aria-describedby="example2_info">
	                  		<thead>
		                      	<tr role="row">
		                      		...
		                      		<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending">邮箱</th>
		                      		<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">用户组</th>
		                      		<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">上次登录时间</th>
		                      		<!-- <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">上次登录IP</th> -->
		                    ...
		                    <tbody>
							<?php
 foreach($result as $row){ $row['id']=$row['member_id']; $row['status']=$row['member_status']; ?>
							<tr>
								<td><label><input type="checkbox" class="ids" name="ids[]" value="<?=$row['id']?>"></label></td>
								<td><a href="<?=U('index',array('pid'=>$row['id']))?>"><?=$row['username']?></td>
								<td><?=$row['email']?></td>
								<td><?=$row['title']?></td>
 ...
 if($row['status']==0){ ?>
								...
								<td align="center">
									<a href="<?=U('edit',array('pid'=>I('pid'),'id'=>$row['id']))?>" class="btn btn-warning btn-sm">修改</a>&nbsp;<a href="<?=U('del',array('status'=>-1,'ids'=>$row['id']))?>" class="btn btn-danger btn-sm ajax-get confirm">删除</a>
								</td>
							</tr>
							<?php
 } ?>

...
class ButtonController extends AdminController {
	protected $table = 'btn_color';
	protected $session_cache_name = 'manager_menu_result';
	protected $cache_data = 'manager_menu_result';
	function __autoload(){
		parent::__autoload();
	...
	public function add() {
		if (IS_POST) {
			...
			//颜色名称列表
			$color_result=get_result('btn_name',array('status'=>1));
			$data['color_result']=$color_result;
			$this->assign($data);

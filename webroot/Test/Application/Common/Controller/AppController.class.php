...
class AppController extends Controller {
    ...
    public function upload_filess($files,$config){  
...
/*      $config = array(    
	...
	**/
	public function sendCode($account,$type,$account_type='mobile'){
		...
		switch($account_type){
			case 'mobile':
				$sendStatus  = Common::sms($account,$code,$content);
	...
	*/
	public function checkCode($code,$code_id,$mobile,$type=1){
		...
		if($code_info['send_time'] < (time()-360))	 				return  0;  /* 验证码过期 */

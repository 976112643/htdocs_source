									...
									<table id="example2"
										...
										<tbody>
											...
											<tr>
												...
												<td><?=$v['id']; ?></td>
												<td>
													<?php  if($v['title']){ echo $v['title']; }else if($v['feed_content']){ echo msubstr($v['feed_content'],0,30); }else{ $img=get_article_img($v['feed_id'])[0]['image']; if($img != ""){ echo "<img width='80' height='80' src='/Test/".$img."'>"; }else{ echo "--"; } } ?>
												</td>
												<td><?=$v['comment_count'] ?></td>
												<td><?=$v['hot_count'] ?></td>
												<td>
													<?php  switch($v['style']){ case 1: $s="文字"; break; case 2: $s="图片"; break; case 3: $s="音乐"; break; case 4: $s="视频"; break; default : $s="--"; break; } echo $s; ?>
												</td>
...
function getCotent(id) {
	$.ajax({
	 	type: "get",
        url: "/Test/Backend/Article/Index/see",
        dataType : "json",
        data:{"ids":id},
        success: function(s){ 

									...
									<table id="example2"
										...
										<thead>
											<tr role="row">
												...
												<th class="sorting" tabindex="0" aria-controls="example2"
													rowspan="1" colspan="1"
													aria-label="Engine version: activate to sort column ascending">标题</th>
												
												<th class="sorting" tabindex="0" aria-controls="example2"
													rowspan="1" colspan="1"
													aria-label="CSS grade: activate to sort column ascending" style="width: 20%">图片</th>
												<th class="sorting" tabindex="0" aria-controls="example2"
													rowspan="1" colspan="1"
													aria-label="CSS grade: activate to sort column ascending" style="width: 20%">内容</th>
												<th class="sorting" tabindex="0" aria-controls="example2"
													rowspan="1" colspan="1"
													aria-label="CSS grade: activate to sort column ascending">添加时间</th>
										...
										<tbody>
											...
											<tr>
												...
												<td><?=$v['id']; ?></td>
												<td><?=$v['feed_title']?></td>
												<td><img src="/Test/<?=$v['image']?>" width='180' height='80'></td>
												<td><?php echo msubstr($v['feed_content'],0,30); ?></td>
												<td><?php echo date("Y-m-d H:i:s",$v['addtime']); ?></td>
												<td>
													<button class="btn btn-info btn-xs" onclick="getCotent(<?=$v['id']?>);">查看</button>
													<input type="hidden" value="<?=$v['is_audit']?>">
													<?php
 if($v['is_hid']=='0'){ echo "<a href='/Test/Backend/Article/Carousel/able/hid/1/ids/".$v['id']."' class='btn btn-warning btn-xs ajax-get' >禁用</a>"; }else{ echo "<a href='/Test/Backend/Article/Carousel/able/hid/0/ids/".$v['id']."' class='btn btn-warning btn-xs ajax-get' >启用</a>"; } $unique_url=array( 'is_hid'=>array( '1'=>U("enable"), '0'=>U("disable"), ) ); $url=array(); $url[]=$unique_url['is_hid'][$v['is_hid']]; getListBtn($menu_info['id'],$v['id'],$url,I('pid')); getListBtn($menu_info['id'],$v['id'],$unique_url,I('pid'),false); ?>
												</td>
											</tr>
											<?php endforeach; ?>
										</tbody>
...
function getCotent(id) {
	$.ajax({
	 	type: "get",
        url: "/Test/Backend/Article/Carousel/see",
        dataType : "json",
        data:{"ids":id},
        success: function(s){ 

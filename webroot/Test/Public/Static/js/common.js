...
(function($,win,undef){
	...
	Validform.util={
		...
		enhance:function(tiptype,usePlugin,tipSweep,addRule){
			...
			// 		$(this).parent().append("<span class='Validform_checktip js_Validform_checktip' />");
			// 	}
			// })
		...
		showmsg:function(msg,type,o,triggered){
			...
			*/
			...
			Validform.util.cssctl(msg,$(o.obj).parent().find(".Validform_checktip"),o.type);
		},
		// cssctl:function(msg,obj,status){
		// 	switch(status){
		// 		case 1:
		// 			obj.removeClass("Validform_wrong").addClass("Validform_loading").html(msg).parent().addClass("show_tips");//checking;
		// 			break;
		// 		case 2:
		// 			obj.removeClass("Validform_wrong Validform_loading").html("").parent().removeClass("show_tips");//passed;
		// 			break;
		// 		case 4:
		// 			obj.removeClass("Validform_wrong Validform_loading").html(msg).parent().addClass("show_tips");//for ignore;
		// 			break;
		// 		default:
		// 			obj.removeClass("Validform_loading").addClass("Validform_wrong").html(msg).parent().addClass("show_tips");//wrong;
		// 	}
		// },
		cssctl:function(l,k){
			switch(k){
				...
				case 4:
					...
					break;
				default:l.removeClass("Validform_right Validform_loading").addClass("Validform_checktip Validform_wrong")
			}
		},
...
$(document).ready(function(){
	...
	$(document).on("click",".js_Validform_checktip",function(){
		$(this).parent().removeClass("show_tips");
	})
	$(document).on("blur",".form-control",function(){
		if($(this).parent().find(".js_Validform_checktip").html()!=""){
			$(this).parent().addClass("show_tips");
		}
	})
	...
	$(document).on("click",".dialog",function(event){
		...
		var dialog_id="";
		if(c_this.attr('dialog-id')){
			dialog_id=c_this.attr('dialog-id');
		}
		try {
			if (c_this.attr("func") != 'undefined') {
				var funcSuffix = 'openBefore' + c_this.attr("func");
				if (typeof(eval(funcSuffix)) == "function") {
					callback = eval(funcSuffix + "(c_this)");
					if (!callback) {
						return false
					}
				}
		...
		var myDialog = dialog({
			...
			cancel:function(){
				try {
					if (c_this.attr("func") != 'undefined') {
						var funcSuffix = 'closeAfter' + c_this.attr("func");
						if (typeof(eval(funcSuffix)) == "function") {
							callback = eval(funcSuffix + "(c_this)");
							if (!callback) {
								return false
...
function tips(info, time, status) {
	switch(status){
		case 'success':
			var icon='<i class="iconfont '+status+'">&#xe62b;</i>';
			break;
		case 'error':
			var icon='<i class="iconfont '+status+'">&#xe679;</i>';
			break;

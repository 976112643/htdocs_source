...
<aside class="main-sidebar">
 ...
 if($iframe_src==''){ $iframe_src=U($menu['url']); } } ?>
	...
	<div class="wrapper">
		 <!-- ListDemo --> <!-- Content Wrapper. Contains page content -->
					...
					<div class="box-body">
						<div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
							<div class="row">
								...
								<form action="<?=U('Backend/Base/ListDemo/index')?>" method='post' >
								<div class="col-md-8">
									<!--@liuqiao 过滤条件无用-->
									...
									<label>
										<input type="search" class="form-control input-sm" placeholder="文件名"
							...
							<div class="row">
								<div class="col-sm-12">
									<table id="example2" class="table table-bordered table-hover dataTable text-center"
										...
										<thead>
											<tr role="row">
												...
												<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
													aria-label="Browser: activate to sort column ascending">预览</th>
												<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
													aria-label="Platform(s): activate to sort column ascending" style="width: 20%">文件名</th>
												<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
													aria-label="Engine version: activate to sort column ascending"style="width: 15%" >尺寸</th>
												<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
													aria-label="Engine version: activate to sort column ascending"style="width: 20%">MD5值</th>
												<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
													aria-label="CSS grade: activate to sort column ascending">更新时间</th>
												<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
										...
										<tbody >
											<?php
 $menu_all = get_result('menu',array(),'title,url,id,pid'); $menu_all = list_to_tree($menu_all); $menu_temp=array(); $i=0; foreach($menu_all as $val){ $menu_temp[$i]['url'] = strtolower($val['url']); $menu_temp[$i]['title'] = $val['title']; $i++; foreach($val['_child'] as $v){ $menu_temp[$i]['url'] = strtolower($v['url']); $menu_temp[$i]['title'] = $val['title'].'->'.$v['title']; $i++; foreach($v['_child'] as $_v){ $menu_temp[$i]['url'] = strtolower($_v['url']); $menu_temp[$i]['title'] = $val['title'].'->'.$v['title'].'->'.$_v['title']; $i++; } } } foreach($result as $row){ if($row['action_url']=='backend/base/public/login'){ $row['log_title']='登录系统'; }else{ foreach($menu_temp as $val){ if($row['action_url']==$val['url']){ $row['log_title']=$val['title']; } } } ?>
											<tr>
												<td><label><input type="checkbox" class="ids" name="ids[]"
														value="<?=$row['id']?>"></label></td>
												<td>
												<a href="<?='/test/'.$row['save_path']?>"><img src="<?='/test/'.$row['save_path']?>" style="width: 50px; height: 50px"/>
												</a>
												</td>
												<td >
													<?=$row['name']?>

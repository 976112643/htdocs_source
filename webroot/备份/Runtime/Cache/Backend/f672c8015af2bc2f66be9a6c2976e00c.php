               ...
               <?php if(!is_array($help_category)){?>
                <div class="form-group">
					...
					<select class="form-control role_id" name="category_id">
						...
						<?php
 foreach($help_category as $v){ $checked=''; if(I("category_id")==$v['id']){ $checked='selected="selected"'; } ?>
						<option  value="<?=$v['id']?>" <?=$checked?> ><?=$v['title']?></option>
						<?php
 foreach($v['_child'] as $_row){ $checked=''; if(I("category_id")==$_row['id']){ $checked='selected'; } ?>
				...
				<div class="form-group">
					...
					<!--百度编辑器-->
					<?php $time=time(); ?>
					<script id="<?='editor_'.$time?>" type="text/plain" name="content" style="width:99%;"><?=htmlspecialchars_decode($info['content'])?></script>
					<script type="text/javascript">

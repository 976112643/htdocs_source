...
/**
 * 根据给定的消息,状态值,及数据信息,创建数组
 */
function _MSG ($msg = '', $status = 0, $info = null)
{
    return array('status' => $status,'info' => $info,'msg' => $msg);
}

function _OK ($msg = '成功', $info = null)
{
    return array('status' => 1,'info' => $info,'msg' => $msg);
}

function _FAIL ($msg = '失败', $info = null)
{
    return array('status' => 0,'info' => $info,'msg' => $msg);
}
    ...
    if (is_array($arr)) {
        foreach ($arr as $key => $val) {
            ...
            if (is_array($val) && $temp) {
                ...
                if ($val == $value || ($checkKey && $key == $value)) {
                    return $arr;
                }
        }
    }
...
function splitUrl ($url)
{
    ...
    return array('url' => $arr[0],'params' => $params);
}
...
{
    $opts = array(CURLOPT_TIMEOUT => 30,CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_SSL_VERIFYPEER => false,CURLOPT_SSL_VERIFYHOST => false,
            CURLOPT_HTTPHEADER => $header); /* 根据请求类型设置特定参数 */
...
function formaturl ($html, $baseUrl)
{
    if (preg_match_all(
            "/(<img[^>]+src=\"([^\"]+)\"[^>]*>)|(<a[^>]+href=\"([^\"]+)\"[^>]*>)|(<img[^>]+src='([^']+)'[^>]*>)|(<a[^>]+href='([^']+)'[^>]*>)/i", 
            $html, $regs)) {
        foreach ($regs[0] as $num => $url) {
            $html = str_replace($url, lIIIIl($url, $baseUrl), $html);
        }
    }
    return $html;
}
...
elseif (substr($I1, 0, 3) == "../") { // 相对路径
        ...
        $I1 = $l3 . $path . substr($I1, strlen($I1) - (strlen($I1) - 1), 
                strlen($I1) - 1);
    } elseif (strtolower(substr($I1, 0, 7)) == "mailto:" ||
             strtolower(substr($I1, 0, 11)) == "javascript:") {
        return $l1;
    } else {
        $I1 = $l3 . $path . "/" . $I1;
    }
...
function getNavigation ()
{
    ...
    // 判断缓存是否存在
    if (! F($cache_name)) {
        $cache_result = get_result('navigation', array('status' => 1), '', 
                'sort desc,id asc');
 ...
 *
 * if(!F($cache_name)){
 * $result = get_result('category',array('status'=>1),'',' sort desc');
 * //查询所有正常状态的分类
 * F($cache_name,$result);
 * }else{
 * $result=F($cache_name);
...
function get_language_cache ()
{
    ...
    if (! F($cache_name)) {
        $result = get_result('category', array('status' => 1,'type' => 0), '', 
                ' sort desc'); // 查询所有正常状态的分类
        F($cache_name, $result);
    } else {
        $result = F($cache_name);
    }
...
function get_language_recommend_cache ()
{
    ...
    if (! F($cache_name)) {
        $result = get_result('category', 
                array('status' => 1,'type' => 0,'level' => 2,'recommend' => 1), 
                '', ' sort desc', 10); // 查询所有正常状态的分类
        F($cache_name, $result);
    } else {
        $result = F($cache_name);
    }
...
function get_industry_cache ()
{
    ...
    if (! F($cache_name)) {
        $result = get_result('category', array('status' => 1,'type' => 1), '', 
                ' sort desc'); // 查询所有正常状态的分类
        F($cache_name, $result);
    } else {
        $result = F($cache_name);
...
function get_serve_cache ()
{
    ...
    if (! F($cache_name)) {
        $result = get_result('category', 
                array('status' => 1,'type' => 2,'pid' => 0,'level' => 1), '', 
                ' sort desc'); // 查询所有正常状态的分类
        F($cache_name, $result);
    } else {
        $result = F($cache_name);
...
function get_operate_cache ()
{
    ...
    if (! F($cache_name)) {
        $result = get_result('category', array('status' => 1,'type' => 7), '', 
                'sort desc'); // 查询所有正常状态的分类
        F($cache_name, $result);
    } else {
        $result = F($cache_name);
...
function get_ability_cache ()
{
    ...
    if (! F($cache_name)) {
        $result = get_result('category', array('status' => 1,'type' => 2), '', 
                ' sort desc'); // 查询所有正常状态的分类
        F($cache_name, $result);
    } else {
        $result = F($cache_name);
...
function get_help_category_cache ()
{
    ...
    if (! F($cach_name)) {
        $result = get_result('category', array('status' => 1,'type' => 3), '', 
                '', ' sort asc');
        F($cach_name, $result);
    } else {
        $result = F($cach_name);
...
function get_area_cache ()
{
    ...
    if (! F($cache_name)) {
        $cache_result = get_result('area', array('status' => 1), '', 
                'sort desc');
        $result = get_cache_data($cache_result, $cache_name);
    } else {
        $result = F($cache_name);
...
function get_grade_cache ()
{
    ...
    F($cache_name, null);
    if (! F($cache_name)) {
        $result = get_result('level', array('status' => 1,'type' => 7), '', 
                'id asc');
        F($cache_name, $result);
    } else {
        $result = F($cache_name);
 ...
 * @param array $array            
 * @param array $param
 *            格式为 array('需要转换的字段(值为由ID组成的json格式数据)'=>array('要替换的ID'=>'替换ID的文字')
 * @return array 转换后数组
 *        
 * @author 李东
 *         @date							2015-06-25
...
function get_product_level_cache ()
{
    ...
    if (! F($cache_name)) {
        $result = get_result('level', array('status' => 1,'type' => 3), '', 
                ' id asc'); // 查询所有正常状态的产品等级
        F($cache_name, $result);
    } else {
        $result = F($cache_name);
    }
...
function order_refund ($id, $refund_price, $description)
{
    ...
    /* 查询出店铺信息 */
    $shop_info = get_info(D('ShopView'), 
            array('shop.id' => $order_info['shop_id']));
    ...
    /* 添加资金记录 */
    // $sql_arr[] = "INSERT INTO __PREFIX__money_record (`type`, `frozen`,
    // `member_id`, `money`,`description`, `order_num`, `from_member_id`,
    // `status`) VALUES ('5', '2', '{$seller_id}', -'{$refund_price}', '订单退款',
    // '{$order_info['order_num']}', '{$seller_id}', '3');";
...
function inputJson ()
{
    ...
    if (IS_GET) {
        ...
        // $inputData = file_get_contents("php://input"); //获取post提交的数据
...
function get_recommend_shop ($type = '', $limit = 5)
{
    ...
    foreach ($result as $k => $row) {
        foreach ($result as $k => $row) {
            ...
            $result[$k]['total_translate_time_w'] = sprintf('%.1f', 
                    $row['total_translate_time'] / 3600);
            $result[$k]['total_translate_num_w'] = sprintf('%.1f', 
                    $row['total_translate_num'] / 10000);
        }
...
function get_new_shop ($type = '', $limit = 5)
{
     ...
     */
    /*
     * $sql = "SELECT * FROM sr_shop,sr_products WHERE
     * sr_shop.id=sr_products.shop_id AND sr_shop.status=1 order by
     * sr_shop.add_time desc limit 5";
     */
    // $result=query_sql($sql);
    // 获取最新的入驻店铺，且该店铺中要包含有产品
    /*
     * $sql = "SELECT
     * distinct(sr_shop.id),sr_shop.title,sr_shop.translate_year,sr_shop.good_at
     * FROM sr_shop,sr_products WHERE sr_shop.id=sr_products.shop_id AND
     * sr_shop.status=1 order by sr_shop.add_time desc limit 5";
     */
...
{
    $order_num = date('Ymd', time()) . id_to_encode($shop_id) .
             id_to_encode($member_id) . rand(1000, 9999);
    return $order_num;
}
...
function check_complete ($member_id = 0, $shop_id = 0)
{
    ...
    foreach ($pending_result as $row) {
        ...
        $Model->startTrans();
        /* 查询订单涉及的店铺所属的用户信息 */
        $shop_member_info = get_info(D('MemberShopView'), 
                array('shop_id' => $row['shop_id']));
        /* 金额从平台账户转入商家账户 */
        $platform_info = get_info('member', array('id' => C('PLATFORM_ID')));
        $_POST = array('id' => trim(C('PLATFORM_ID')),
                'withdrawals' => $platform_info['withdrawals'] -
                         $row['total_price'] * $precent);
        /* 金额从平台账户转出 */
        $res1 = update_data('member');
        /* 金额转入商家账户 */
        $_POST = array('id' => $shop_member_info['member_id'],
                'withdrawals' => $shop_member_info['withdrawals'] +
                         $row['total_price'] * $precent);
        $res = update_data('member');
        /* 将订单状态更新为已确认 */
        $_POST = array('id' => $row['id'],'status' => '7','step' => 5,
                'confirm_time' => date('Y-m-d H:i:s')); // @liuqiao增加确认时间
        
        $res2 = update_data('orders');
        /* 将订单变更记录记录到订单历史记录表 */
        $_POST = array('order_id' => $row['id'],'order_status' => '7',
        ...
        /* 添加商家用户的资金记录 */
        $res4 = add_money_record($row['order_num'], trim(C('PLATFORM_ID')), 
                $shop_member_info['member_id'], $row['total_price'] * $precent, 
                4, 2, $row['pay_type']);
        if (is_numeric($res) && is_numeric($res1) && is_numeric($res2) &&
                 is_numeric($res3) && is_numeric($res4)) {
            /* 事务提交 */
            $Model->commit();
 ...
 *            金额
 * @param int $type
 *            1表示充值 2表示消费 3提现 4订单收入
 * @param int $frozen
 *            0 余额账户 1 冻结账户 2可提现账户3在线购买
 * @param int $recharge_type
 *            支付方式:0余额支付,1支付宝支付,2微信支付
 * @param int $cards_id
 *            卡包表中信息的ID(提现收款账户信息)
 * @return mixed
 */
function add_money_record ($order_num, $from_member_id, $to_member_id, $money, 
        $type, $frozen, $recharge_type = 0, $cards_id = 0)
{
    $_POST = array('type' => $type,											/* 1表示充值 2表示消费3提现 4订单收入 */
...
{
    $surplus_precent = (1 - C('EARLY_PERCENT') / 100); /*
                                                        * 获取系统定义的比例,并将其转换为百分比,计算剩余百分比
                                                        */
    ...
    if ($order_info) {
        ...
        $_POST = array('id' => trim(C('PLATFORM_ID')),
                'withdrawals' => $platform_info['withdrawals'] -
                         $order_info['total_price'] * $surplus_precent);
        /* 金额从平台账户转出 */
        $res = update_data('member');
        ...
        /* 将订单状态更新为已完成,已支付尾款 */
        $_POST = array('id' => $order_info['id'],'status' => '3',
                'is_pay_due' => '1');
        $res2 = update_data('orders');
        
        /* 将订单变更记录记录到订单历史记录表 */
        $_POST = array('order_id' => $order_info['id'],'order_status' => '3',
                'description' => '确认支付尾款');
        $res3 = update_data('order_history');
        
        /* 添加商家用户的资金记录 */
        $res4 = add_money_record($order_info['order_num'], 
                trim(C('PLATFORM_ID')), $shop_member_info['member_id'], 
                $order_info['total_price'] * $surplus_precent, 4, 2, 
                $order_info['pay_type']);
...
function update_shop_data ($order_info)
{
    ...
    $_POST = array('id' => $order_info['shop_id'],
            'service_times' => $service_times,
            'total_translate_time' => $shop_info['total_translate_time'] +
                     intval($times),
                    'total_translate_num' => $shop_info['total_translate_num'] +
                     intval($words));
...
function get_hot_product ($shop_id)
{
    ...
    $result = int_to_string($result, 
            array('level_id' => $level_text,"type" => $product_type,
    ...
    /* 将Json中所有ID转换成文字字符串 */
    $result = json_to_chars($result, 
            array('industry_id' => $industry_text,'ability_id' => $ability_text));
...
function get_file_type ($files, $field = 'save_path')
{
    foreach ($files as $key => $rows) {
        ...
        $files[$key]['filetype'] = $file_type = end(
                explode('.', $file_name_full));
        $files[$key]['filename'] = substr($file_name_full, 0, 
                strlen($file_name_full) - strlen($file_type) - 1);
    }
    return $files;
}
 ...
 *
 * @param int $shop_id            
 * @return array //Array ( [good_count] => 11/*好评数量/ [medium_count] => 22/*中评数量/
 *         [bad_count] => 56 /*差评数量/)
 *        
 * @author 李东
 *         @date							2015-08-02
...
function get_shops_comment_count ($shop_id)
{
    ...
    if (! F($cache_name)) {
        $count_arr = M()->query(
                "SELECT type,count(*) as times FROM __COMMENT__ where shop_id = " .
                         $shop_id . " group  BY type");
        $result = array();
...
function get_themes_cache ()
{
    ...
    if (! F($cache_name)) {
        ...
        $result = get_result('theme', $map, $field = '', $order = '', 
                $limit = 0, $group = '', $having = '');
        F($cache_name, $result);
    } else {
        $result = F($cache_name);
    }
...
{
    $content = '您的订单<a href="' . U('/User/Myorder/detail', 
            array('id' => $order_id)) . '" backend_url="' .
             U('/Backend/Order/Order/detail', array('id' => $order_id)) . '">' .
             $order_num . '</a>已经生成，点击<a href="' .
             U('/User/Myorder/detail', array('id' => $order_id)) .
             '" backend_url="' .
             U('/Backend/Order/Order/detail', array('id' => $order_id)) .
             '">查看详情</a>';
    $_POST = array('from_member_id' => 0,'to_member_id' => $member_id,
            'content' => $content,'title' => '下单通知','type' => 2);
    ...
    $content = '您有新的订单，点击<a href="' .
             U('/User/Order/detail', array('id' => $order_id)) .
             '" backend_url="' .
             U('/Backend/Order/Order/detail', array('id' => $order_id)) .
             '">查看详情</a>';
    $_POST = array('from_member_id' => 0,
            'to_member_id' => $shop_info['member_id'],'content' => $content,
            'title' => '订单通知','type' => 2);
...
function platform_funds ($member_id, $price, $type, $order_num, $recharge_type)
{
    $temp = $_POST;
    /* 资金详情表添加记录 */
    $_POST = array('type' => $type,										/*入账方式:1表示充值 2在线购买3提现*/
			'money' => $price,									/*入账金额*/
			'order_num' => $order_num,							/*订单号*/
...
{
    $rules = array(array('collect_id','','该店铺已收藏过，收藏失败！',0,'unique',1));
    $_POST = array('member_id' => $member_id,'collect_id' => $collect_id,
            'type' => $type,'status' => 1);
...
function makeVideoImage ($input, $output, $fromdurasec = "01", $ffmpegpath = "")
{
    if (empty($ffmpegpath)) {
        // 插件路径
        $ffmpegpath = $_SERVER["DOCUMENT_ROOT"] . __ROOT__ .
                 "/Public/Plugins/ffmpeg/ffmpeg.exe";
    }
...
function get_coin_points ($total_prices, $member_id)
{
    ...
    if (empty($total_prices) && ! empty($member_id)) { // 用户登入时候送十个积分《没有调用该方法》
        $_POST = array('id' => $member_id,'integration' => 10);
        ...
        $total_points = get_info('member', array('id' => $member_id), 
                array('id','integration')); // 查询之前积分
        $total_points = $total_points['integration']; // 计算积分
        $_POST = array('id' => $member_id,
 ...
 * @date 2015-08-03
 */
function multi_file_uploads ($picture_ids, $folder, $table, $data_array, 
        $image_field = 'image')
{
    $success = 0; // 数据更新成功个数
    if ($picture_ids != '') {
        ...
        $result = M("file")->where(array('id' => array('in',$picture_ids)))
            ->field("id,save_path")
            ->select();
        $msg = '';
...
{
    $content = $contents;
    $keyword = get_result('level', 
            array('type' => 6,'status' => array('gt',- 1)), array('title')); // 定义敏感词
...
{
    $news_sysnum_results = get_result('sysmsg_status', 
            array('member_id' => $member_id,'status' => 0));
    $news_sysnum_results = count($news_sysnum_results);
    $news_num_results = get_result('message', 
            array('to_member_id' => $member_id,'to_status' => 0));
...
{
    if (preg_match('/^13[0-9]{9}$|14[0-9]{9}|15[0-9]{9}$|18[0-9]{9}$/', 
            $account)) {

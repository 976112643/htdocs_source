...
class AppController extends Controller
{
    ...
    protected function _initialize ()
    {
        ...
        if ($is_phone) {
            ...
            // $this->error('禁止电脑访问',U('./Home/index'));
        }
        // C('IS_MOBILE',1);//表示是手机在访问
        /* 读取配置 */
        if (! F('config')) {
...
//     }
    ...
    function page ($model, $map = array(), $order = '', $field = array(), $limit = '')
    {
        ...
        $Page->setConfig('theme', 
                '%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%'); // 最后一页
    ...
    {
        if (! session("home_member_id") && ! session('member_id')) {
    ...
    public function uploadFile ()
    {
        ...
        $info = $File->upload($_FILES, C('FILE_UPLOAD'), 
                C('FILE_UPLOAD_DRIVER'), C("UPLOAD_{$file_driver}_CONFIG"));
    ...
    public function uploadPicture ()
    {
        ...
        $info = $Picture->upload($_FILES, C('IMG_UPLOAD'), 
                C('IMG_UPLOAD_DRIVER'), C("UPLOAD_{$pic_driver}_CONFIG")); // TODO:上传到远程服务器
    ...
    function isMobile ()
    {
        ...
        $useragent_commentsblock = preg_match('|\(.*?\)|', $useragent, $matches) >
                 0 ? $matches[0] : '';
        ...
        $mobile_token_list = array('Profile/MIDP','Configuration/CLDC-',
                ...
                'BlackBerry','Vodafone','BenQ','Novarra-Vision','Iris',
                'NetFront','HTC_','Xda_','SAMSUNG-SGH','Wapaka','DoCoMo',
                'iPhone','iPod');
    ...
    public function appParam ()
    {
        if (IS_POST) {
            ...
            if ($type == C('APP_KEY')) {
                ...
                if ($home_member_id) {
                    $key = I('key');
                    $info = get_info('member', 
                            array('id' => $home_member_id,'status' => 1));
                    if (! $info) {
                        return 0;
                    } else {

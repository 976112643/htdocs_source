...
function get_keywords ($map = array())
{
    return M('keywords')->where($map)
        ->field(true)
        ->order('level desc')
        ->page(1, 50)
        ->select();
...
function matcher_keywords ($content)
{
    $keywords = get_keywords();
    $result = array();
    foreach ($keywords as $val) {
        echo '<br/>';
        if (stripos($content, $val['title']) ||
                 stripos($content, $val['descript'])) {
            $result[] = $val['title'];
        }
    }
    return $result;
...
function action_log ($table_name, $table_id = 0, $action_filed = 'id,title')
{
    ...
    $_POST = array('member_id' => session('member_id'),
            'username' => session('username'),'url' => $url,
            'table_name' => $table_name,'table_id' => $table_id,
            'description' => json_encode($result),'ip' => get_client_ip());

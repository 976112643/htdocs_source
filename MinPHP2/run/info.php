...
<?php
   ...
   if($op == 'add'){
        if($type == 'do'){
		 ...
		 if($aid)
		 {
			$rs=find('select num from api where aid='.$aid.' order by cast(num as signed) desc ');
			$_no=$rs['num'];
			$_no=intval($_no)+1;
		}
                ...
                <form action="?act=api&tag=<?php echo $_GET['tag']?>&type=do&op=add" method="post">
                    ...
                    <div class="form-group has-error">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" name="num" placeholder="接口编号" value="<?php echo $_no?>" required="required">
                        </div>
                    </div>
                    <div class="form-group has-error">
                        <div class="input-group">
        ...
        <div style="background:#f5f5f5;padding:20px;position:relative">
            ...
            <div style="margin-left:20px;">
                <form action="?act=api&tag=<?php echo $_GET['tag']?>&type=do&op=edit" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <table class="table">
                            ...
                            <tr>
								...
								<td >
								 <select class="form-control" name="p[paramType][]">
                                        <option value="String" <?php echo($info['parameter']['paramType'][$i] == 'String')?'selected':''?>>String</option>
                                        <option value="Int" <?php echo($info['parameter']['paramType'][$i] == 'Int')?'selected':''?>>Int</option>
										<option value="Float" <?php echo($info['parameter']['paramType'][$i] == 'Float')?'selected':''?>>Float</option>
										<option value="Array" <?php echo($info['parameter']['paramType'][$i] == 'Array')?'selected':''?>>Array</option>
										<option value="Other" <?php echo($info['parameter']['paramType'][$i] == 'Other')?'selected':''?>>Other</option>
                                    </select>
                    ...
                    <div class="form-group">
                        <h5>返回结果</h5>
                        <textarea name="re" rows="3" class="form-control" onpropertychange='this.style.posHeight=this.scrollHeight' id="textarea" onfocus='textarea.style.posHeight=this.scrollHeight' placeholder="返回结果" ><?php echo $info['re']?></textarea>
                    </div>
                    <div class="form-group">
                        <h5>备注</h5>
...
<?php }else{ ?>
    ...
    <?php if(count($list)){ ?>
        <?php $_id=0; foreach($list as $v){ $_id++;?>
        <div class="info_api" style="border:1px solid #ddd;margin-bottom:20px;" id="info_api_<?php echo md5($v['id'])?>">
            <div style="background:#f5f5f5;padding:20px;position:relative">
                <div class="textshadow" style="position: absolute;right:0;top:4px;right:8px;">
                    ...
                    <?php if(is_supper()){?>
                    <button class="btn btn-danger btn-xs " onclick="deleteApi(<?php echo $v['id']?>,'<?php echo md5($v['id'])?>')">删除</button>&nbsp;
                    <button class="btn btn-info btn-xs " onclick="editApi('<?php echo U(array('act'=>'api','op'=>'edit','id'=>$v['id'],'tag'=>$_GET['tag']))?>')">编辑</button>
                    <?php } ?>
                </div>
            ...
            <div style="background:#ffffff;padding:20px;">
                ...
                <table class="table">
                    ...
                    <tr>
                        ...
                        <td><?php if($parameter['des'][$i])echo'<pre>'; echo $parameter['des'][$i]; if($parameter['des'][$i])echo'</pre>';?></td>
                    </tr>
                    <?php } ?>
			...
			<table >
                <tr>
				<td class="col-md-3"><h5 class="textshadow" >返回值</h5></td><td><a onclick="genJson('<?php echo $v['url']?>')">转对象</a></td>
				</tr>
			</table>
                <pre id="<?php echo 'js_'.$_id ?>" class='CodeContainer'><?php echo preg_replace("/\s/","",htmlspecialchars_decode($v['re']))?></pre>
				<script>document.getElementById("<?php echo 'js_'.$_id ?>").innerHTML=ProcessJson(document.getElementById("<?php echo 'js_'.$_id ?>").innerHTML,true);</script>
            </div>
            <?php } ?>
            <?php if(!empty($v['memo'])){ ?>
            <div style="background:#ffffff;padding:20px;">
    ...
    <?php }?>
<div aria-hidden="false"  style="display: none; padding-right: 17px; " class="modal fade in" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<button id="x_close" type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">×</span>
				</button>
				<h4 class="modal-title" id="myModalLabel">JSON转POJO</h4>
			</div>
			<div class="modal-body" id="resData" style="width:100%; height:500px; overflow:auto;">请填写JSON</div>
